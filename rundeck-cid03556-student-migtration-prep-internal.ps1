...
#>
  ...
  Write-Host "Checking if legacy mail: $LegacyUserMail  like: $GoogleWorkspaceSourceMailDomain"
    if ( $LegacyUserMail -like "*$GoogleWorkspaceSourceMailDomain" ) {
        ...
        Write-Host "Invoke-Expression $GamDir\gam.exe update user $LegacyUserMail email $ReplacementUserMail firstname '$FirstName' lastname '$LastName' org '$GoogleWorkspaceDestinationUserOU/$UpdatedDestOU' $GoogleCustomAttribute01 $UPN gal $GoogleIncludeInGal"
        ...
        $AccountHistory = "Migrated"
            ...
            Write-Host "Invoke-Expression $GamDir\gam.exe create user $ReplacementUserMail firstname '$FirstName' lastname '$LastName' org '$GoogleWorkspaceDestinationUserOU/$UpdatedDestOU' $GoogleCustomAttribute01 $UPN password $password gal $GoogleIncludeInGal"
            $LegacyUserMail = "N/A"
            $AccountHistory = "New"
    ...
    Invoke-Expression "$GamDir\gam.exe user $GoogleSvcAccount update drivefile id $UpdatedUsersInfoGsheetID localfile $tempcsv2 newfilename '$GoogleWorkspaceDestinationMailDomain Migrated User Info'" #-ErrorAction SilentlyContinue 
