...
foreach ( $report in $gsheetsData) {
    ...
    $SourceSFTPFileNameCompleteTMP = "$Datadir\TMP_$SourceSFTPfilename"
    DashedLine
        ...
        if ((Get-Item $SourceSFTPFileNameComplete).length -gt 0){
            Write-Host "File: $SourceSFTPFileNameComplete size: $((Get-Item $SourceSFTPFileNameComplete).length) bytes - proceeding with gsheet replacement"
                ...
                if ( $SEL -imatch ";") {
                   ...
                   (get-content $SourceSFTPFileNameComplete) | ForEach-Object {$_ -replace 'ÿ',';' -ExpandProperty } | Out-File $SourceSFTPFileNameCompleteTMP -noclobber -force
                   start-sleep 5
